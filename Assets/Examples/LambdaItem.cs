...
{
    public class LambdaItem : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            EventText = JsonUtility.ToJson(DataSave.Instance._data); 
            if (DataSave.Instance.item.uid != 0&&DataSave.Instance.item.Uuid !=string.Empty)
            {
                if (isOn == false)
                {
                    lambdaPublic.Invoke("AttendDailytQuest2", EventText, "todayQuest");
                    //Invoke();
                    isOn = true;
                }
            }
